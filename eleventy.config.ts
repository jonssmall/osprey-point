...
type CollectionItem = {
  date: Date;
  url: string | false;
  data: Record<string, unknown> & { title?: string; tags?: string | string[] };
};
...
export default function (eleventyConfig: EleventyConfig) {
  ...
  // Every template writes root-absolute paths (href="/services/", not a
  // `url` filter), which only works when the site is served from "/". A
  // GitHub Pages project page is served from "/<repo-name>/" instead, so
  // rewrite them at build time rather than touching every template.
  // Vite-managed asset URLs (under /assets/) are excluded — its own `base`
  // option above already prefixes those correctly, and rewriting them here
  // too, before Vite's post-build HTML pass runs, would make Vite unable to
  // find its own entry points.
  if (PATH_PREFIX) {
  ...
  eleventyConfig.addFilter("readableDate", (value: Date) =>
    DateTime.fromJSDate(value, { zone: TIMEZONE }).toFormat("LLLL d, yyyy"),
  );

  eleventyConfig.addFilter("shortDate", (value: Date) =>
    DateTime.fromJSDate(value, { zone: TIMEZONE }).toFormat("LLL d, yyyy"),
  );
  ...
  eleventyConfig.addFilter("limit", (items: unknown[], count: number) =>
    items.slice(0, count),
  );

  eleventyConfig.addFilter("numberFormat", (value: number) =>
  ...
  eleventyConfig.addCollection("departments", (api) =>
    api
      .getFilteredByGlob("src/departments/*.md")
      .sort((a, b) =>
        String(a.data.title ?? "").localeCompare(String(b.data.title ?? "")),
      ),
  );

  eleventyConfig.addCollection("meetings", (api) =>
    api.getFilteredByGlob("src/government/meetings/*.md").sort(byDateDesc),
  );
